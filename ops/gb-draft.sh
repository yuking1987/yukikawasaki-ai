...
各対象について:
1. そのファイルの「## 元メッセージ」（スレッド全体）を、途中を飛ばさず最初から最後まで読む。本文に「【添付 N件】…→ vault/_attachments/…」があれば、それが先方から届いた素材の実ファイル。素材が揃っているかは文言でなく添付で判断する。本文に添付パスがあれば、判断に関わるかどうかで選り好みせず、Read で実ファイルを毎回すべて開いて実物を確認する（画像＝バナー/ロゴの比率・既存とのバランス、スクショ＝指摘箇所、PDF＝中身を必ず目で見る。開ける形式は例外なく全て開く。Excel は本文に文字が展開済みだが、修正指示は貼り込み画像に描かれていることが多いので『貼り込み画像（N行目付近）→ …』のパスも毎回すべて Read で開いて実物を見る＝生の .xlsx は Read で読めないため、展開テキスト＋抜き出し画像の両方で確認する）。(B) の場合は「## 再考依頼」に書かれた社長の指示に必ず従う。
